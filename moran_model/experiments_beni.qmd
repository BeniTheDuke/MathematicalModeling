...
i_list = np.arange(50, 100, 2.5)
n_of_experiments = 100 #todo increase to 1000
success_rates = []

for i in i_list:
  moran_model = MoranModel(100, i, 0.9, 1, SEED)
  successes = 0
  for _ in range(n_of_experiments):
    if(moran_model.run_until_absorption()[-1]>0):
      successes += 1
...
The effect of the increase of the initial population (i) depends on the total population (n). Even in relative terms. If i is 90% of n, it depends on how high n is. 
Todo explain further

```{python}

i_list = np.arange(100, 200, 5)
n_of_experiments = 100 #todo increase to 1000
success_rates = []

for i in i_list:
  moran_model = MoranModel(200, i, 0.9, 1, SEED)
  successes = 0
  for _ in range(n_of_experiments):
    if(moran_model.run_until_absorption()[-1]>0):
      successes += 1
...
Todo different Ns in the same graph (about 5)
